...
public class GameTree implements GameTreeInterface
{
    ...
    public void generateLevelDF(Stack s, Symbol m)
    {
        ...
        // Increase the level for next level of leaves
        v = getLevel() + 1;

        // Check every Sqaure in the grid for if it is a valid move
        for (int r = MINIMUM; r <= d; r++)
        {
            for (int c = MINIMUM; c <= d; c++)
            {
                ...
                // Valid move if there isn't a clash and is not occupied
                if (!b1.clash(l) && !b1.squareOccupied(l))
                {
                    // Create new game state and push on the stack
                    b2 = (Grid)b1.clone();
                    b2.occupySquare(l, m);
                    t = new GameTree(b2, v);
                    s.push(t);

                    // Set the child of this leaf to the new game state
                    this.setChild(t);
    ...
    public GameTree buildGameDF(Stack s, Symbol m, int d)
    {
        ...
        // If the root tree is empty, stop
        if (isEmpty())
        {
            t = this;
            return t;
        ...
        // If the root tree is stop condition (level = dimension), stop
        if (getLevel() == d)
        {
            t = this;

            // Clear the stack because we have found a solution and there is no reason to continue
            while (!s.isEmpty())
            {
                s.pop();
            }
        ...
        // Generate a new level of game states
        generateLevelDF(s, m);
    ...
    public GameTree buildGameBF(Queue q, Symbol m, int d)
    {
        ...
        if (getLevel() == d)
        {
            ...
            // Clear the queue because we have found a solution and there is no reason to continue
            while (!q.isEmpty())
            {
                q.remove();
            }
        ...
        trace("buildGameBF: buildGameBF generateLevelBF");
        ...
        while (!q.isEmpty())
        {
            ...
            // Recurse with the tree which is rooted at the front item and the current queue
