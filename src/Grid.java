...
public class Grid implements GridInterface, Cloneable
{
    ...
    public Object clone()
    {
        ...
        // Go through every Sqaure in the board and copy its value to the new board
        for (int r = MINIMUM; r <= dimension; r++)
        {
            for (int c = MINIMUM; c <= dimension; c++)
            {
    ...
    public void occupySquare(Location l, Symbol s)
    {
        ...
        else
        {
            ...
            // Clone symbol and apply its location
            m = (Symbol)s.clone();
            m.setLocation(l);

            // Set the symbol on the board to the provided cloned symbol
    ...
    public boolean squareOccupied(Location l)
    {
        ...
        // Check if the Square at the given location is empty and return its result
    ...
    public boolean validMove(Location l)
    {
        ...
        // Check if either row or column is less than the minimum board size
        if (r < MINIMUM || c < MINIMUM)
        {
            b = false;
        }
        // Check if either row or column is greater than the board size
